...
class Rook(Piece):
    ...
    def is_valid_move(self, targetSquare, gamePieces, board, capture=False):
        ...
        # Removed, because Pieces.update() prevents exiting the board
        ## Check that the piece is on the board or if it's moving to the board, if it's not, make the move valid no matter what, if we're not capturing
